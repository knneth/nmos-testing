...
def check_mib_vs_sdp_fmtp_h264(ctx: ValidationContext) -> tuple[bool, str]:
    ...
    if (mask & (1 << ps_bit)) and md.h264_parameter_sets:
        ...
        try:
            sdp_parts = md.h264_parameter_sets.split(",")
            sdp_raw = b"".join(base64.b64decode(p) for p in sdp_parts)
        except Exception:
            mismatches.append("sprop_parameter_sets: SDP base64 decode failed")
            sdp_raw = None
        if sdp_raw is not None and mib_raw != sdp_raw:
            mismatches.append("sprop_parameter_sets: MIB and SDP differ")
