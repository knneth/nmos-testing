...
"""Validate all generated ST 2110-31 AM824 audio test streams.

Run:
  python3 validate_audio_test_streams.py [--output-dir DIR]
      [--family pcm|aac|mixed] [--config NAME] [--verbose] [--full-report]
"""
...
from generate_audio_test_streams import ALL_CONFIGS, DEFAULT_OUTPUT_DIR, config_family, parse_source_filter
...
def validate_stream(
    ...
    *,
    ...
    name = pcap.stem
    validator = SCRIPT_DIR / "ipmx_am824_validate_pcap.py"
    cmd: list[str] = [
        sys.executable,
        str(validator),
        str(pcap),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=f"AM824 streams directory (default: {DEFAULT_OUTPUT_DIR})",
    )
    parser.add_argument(
        "--family",
        type=parse_source_filter,
        default={"pcm", "aac", "mixed"},
        help="Validate only selected families: pcm, aac, mixed, all (default: all)",
    )
    ...
    configs = [config for config in ALL_CONFIGS if config_family(config) in args.family]
    if args.config:
        configs = [config for config in configs if config.name == args.config]
        if not configs:
            raise SystemExit(f"Unknown config '{args.config}'. Available: {', '.join(config.name for config in ALL_CONFIGS)}")
    ...
    for config in configs:
        pcap = args.output_dir / f"{config.name}.pcap"
        sdp = args.output_dir / f"{config.name}.sdp"
        manifest = args.output_dir / f"{config.name}_manifest.json"
        if not pcap.exists():
            print(f"  SKIP {config.name} — PCAP not found")
            results.append(ValidationResult(name=config.name, outcome=Outcome.SKIP))
            continue
        if not sdp.exists():
            print(f"  SKIP {config.name} — SDP not found")
            results.append(ValidationResult(name=config.name, outcome=Outcome.SKIP))
            continue
        if not manifest.exists():
            print(f"  SKIP {config.name} — manifest not found")
            results.append(ValidationResult(name=config.name, outcome=Outcome.SKIP))
            continue
        ...
        total_checks += result.shall_pass + result.shall_fail + result.should_pass + result.should_fail
        if result.outcome == Outcome.PASS:
            ...
            print(f"  ERR  {config.name} — {result.error}")

    passed = sum(1 for result in results if result.outcome == Outcome.PASS)
    failed = sum(1 for result in results if result.outcome == Outcome.FAIL)
    errors = sum(1 for result in results if result.outcome == Outcome.ERROR)
    skipped = sum(1 for result in results if result.outcome == Outcome.SKIP)
