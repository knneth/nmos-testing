...
2) (TP10-1, sec 13.3) — SR clock conversion line:
     ...
     where:
       ...
       SR_CLKatSamplerate= MOD(ptp_time_msw*Fs, 2^32) + INT(ptp_time_lsw*Fs/1e9), wrapped to 32 bits
                           with Fs taken from SDP a=rtpmap payload (e.g., 90000)
        ...
        with open(path, "r", encoding="utf-8", errors="ignore") as f:
            for raw in f:
                ...
                if low.startswith("a=rtpmap:"):
                    ...
                    if m:
                        # pt = m.group(1)  # payload type (unused right now)
                        codec = m.group(2).strip().lower()
                        rate  = _str_to_int(m.group(3))   # ← works for 90000 *and* 48000
                        ch    = _str_to_int(m.group(4)) if m.group(4) else None
                        print("RATE:", rate)
                        out["rtpmap_codec"] = codec
                        out["rtpmap_rate"]  = str(rate) if rate is not None else None
                        if ch is not None:
                            out["rtpmap_channels"] = str(ch)
                    else:
                        # Fallback (very forgiving) if regex didn’t match
                        parts = line.split(None, 1)
                        if len(parts) == 2:
                            payload = parts[1].strip()
                            toks = [t.strip() for t in payload.split()]
                            if toks:
                                main = toks[0]                 # e.g., "L24/48000/2" or "jxsv/90000"
                                segs = [s.strip() for s in main.split('/')]
                                if segs:
...
def _get_ipmx_fields(pkt) -> Dict[str, str]:
    ...
    for canon, field in mapping.items():
...
def _media_type_number(media_info_type: Optional[str]) -> Optional[int]:
    """Extract the numeric media_info_type from display strings like '... (3)' or '3'."""
    if not media_info_type:
        return None
    m = re.search(r"\((\d+)\)\s*$", str(media_info_type)) or re.search(r"(\d+)\s*$", str(media_info_type))
...
def _prepare_ipmx_sdp_lines(ipmx: Dict[str, str], sdp: Dict[str, str]) -> Tuple[str, List[str]]:
    ...
    # media_info_type ↔ rtpmap codec check
